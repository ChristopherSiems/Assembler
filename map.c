...
//
// The memory allocated by this method can be freed by calling the freeMap()
// method
...
char *lookupIndex(map aMap, int i) { return aMap->pairs[i].value; }
...
int insertKey(map aMap, char *key, char *value) {
  if (aMap->mapSize < aMap->maxSize) {
    ...
    aMap->pairs[aMap->mapSize].value =
        (char *)malloc(STR_LENGTH * sizeof(char));
